...
export default {
  ...
  methods: {
    Input() {
      ...
      DB["Alhadthe"].forEach((e) => {
        console.log(`this is max ${document.getElementById("range").value}`);
        if (
          e.range >= this.minPrice &&
          e.range <= +document.getElementById("range").value
        ) {
          console.log(e.range);
        }
      });
    ...
    ShowRange() {
      console.log(DB["Aldawarat"].range < this.maxPrice);
    },
    containerBox() {
